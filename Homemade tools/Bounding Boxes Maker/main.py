...
class BBM(object):
    ...
    def edit(self, img_path):
        ...
        while True:
            ...
            if k == 9:
                ...
                cropped = self.img[self.p1y + 1:self.p2y, self.p1x + 1:self.p2x]

                cv2.imwrite(self.values['folder_save'] + '/' + 'cropped' + '/' + str(self.cont2) + self.img_n[self.img_n.rfind('/') + 1:], cropped)
                self.cropped_names.append(self.values['folder_save'] + '/' + 'cropped' + '/' + str(self.cont2) + self.img_n[self.img_n.rfind('/') + 1:])
                self.cont2 += 1
            ### Enter saves the image with the bouding box and loads next img
            if k == 13:
                self.cont += 1
                self.first = True
            ...
            if k == 27:
                ...
                df = pd.DataFrame(self.dict, columns=list(self.dict.keys()), index=[i[i.rfind('/') + 1:] for i in self.cropped_names[:self.cont]])
    ...
    def transform(self, img_n):
        ...
        if (img.size[0] < 512) or (img.size[1] < 512):
            ...
            while (img.size[0] > 1920) or (img.size[1] > 1080):
